...
import { BackendWakeProvider, useBackendWakeContext } from './context/BackendWakeContext.jsx'
import Dashboard from './pages/Dashboard.jsx'
import SummaryView from './pages/SummaryView.jsx'

function ColdStartBanner() {
  const { showColdStartBanner, dismissBanner, canWakeBackend, handleWakeBackend, wakeBusy } =
    useBackendWakeContext()
  if (!showColdStartBanner) return null
  return (
    <div className="flex flex-col gap-2 border-b border-amber-500/30 bg-amber-500/10 px-3 py-3 text-sm text-amber-100 sm:flex-row sm:items-center sm:justify-between sm:px-4">
      <p className="min-w-0 leading-relaxed">
        後端可能正在喚醒（Render 免費方案），請稍候或點「叫醒後端」。
      </p>
      <div className="flex shrink-0 gap-2">
        <button
          type="button"
          onClick={() => handleWakeBackend().catch(() => {})}
          disabled={!canWakeBackend || wakeBusy}
          className="rounded-lg border border-amber-500/40 bg-amber-500/15 px-3 py-1.5 text-xs hover:bg-amber-500/25 disabled:opacity-50"
        >
          叫醒後端
        </button>
        <button
          ...
          className="rounded-lg px-3 py-1.5 text-xs text-amber-200/80 hover:bg-amber-500/10"
        >
          關閉
        </button>
      </div>
      ...
      <BackendWakeProvider>
        ...
        <footer className="border-t border-zinc-800/60 pt-6 pb-[max(1.5rem,env(safe-area-inset-bottom))] sm:pt-8">
          <div className="mx-auto max-w-6xl break-words px-3 text-xs leading-relaxed text-zinc-500 sm:px-4">
            ...
            <span className="text-zinc-600">有報告處理中時每 5 秒更新</span>
          </div>
        </footer>
      </BackendWakeProvider>
    </div>
